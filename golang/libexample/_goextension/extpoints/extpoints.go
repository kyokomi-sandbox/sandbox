...
var MyPlugins = &myPluginExt{
	newExtensionPoint(new(MyPlugin)),
}

type myPluginExt struct {
	*extensionPoint
}

func (ep *myPluginExt) Unregister(name string) bool {
	return ep.unregister(name)
}

func (ep *myPluginExt) Register(component MyPlugin, name string) bool {
	return ep.register(component, name)
}

func (ep *myPluginExt) Lookup(name string) (MyPlugin, bool) {
	ext, ok := ep.lookup(name)
	return ext.(MyPlugin), ok
}

func (ep *myPluginExt) All() map[string]MyPlugin {
	all := make(map[string]MyPlugin)
