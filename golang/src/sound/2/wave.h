...
void wave_read_8bit_mono(MONO_PCM *pcm, char *file_name)
{
  ...
  pcm->length = data_chunk_size; /* ���f�[�^�̒��� */
  pcm->s = calloc(pcm->length, sizeof(double)); /* �������̊m�� */
  
  for (n = 0; n < pcm->length; n++)
  {
    fread(&data, 1, 1, fp); /* ���f�[�^�̓ǂݎ�� */
    pcm->s[n] = ((double)data - 128.0) / 128.0; /* ���f�[�^��-1�ȏ�1�����͈̔͂ɐ��K������ */
  }
...
void wave_write_8bit_mono(MONO_PCM *pcm, char *file_name)
{
  ...
  for (n = 0; n < pcm->length; n++)
  {
    ...
    data = (unsigned char)((int)(s + 0.5)); /* �l�̌ܓ� */
    fwrite(&data, 1, 1, fp); /* ���f�[�^�̏����o�� */
  }
  
  if ((pcm->length % 2) == 1) /* ���f�[�^�̒�������̂Ƃ� */
  {
    data = 0;
    fwrite(&data, 1, 1, fp); /* 0�p�f�B���O */
  }
...
void wave_read_8bit_stereo(STEREO_PCM *pcm, char *file_name)
{
  ...
  for (n = 0; n < pcm->length; n++)
  {
    fread(&data, 1, 1, fp); /* ���f�[�^�iL�`�����l���j�̓ǂݎ�� */
    pcm->sL[n] = ((double)data - 128.0) / 128.0; /* ���f�[�^��-1�ȏ�1�����͈̔͂ɐ��K������ */
    
    fread(&data, 1, 1, fp); /* ���f�[�^�iR�`�����l���j�̓ǂݎ�� */
    pcm->sR[n] = ((double)data - 128.0) / 128.0; /* ���f�[�^��-1�ȏ�1�����͈̔͂ɐ��K������ */
  }
...
void wave_write_8bit_stereo(STEREO_PCM *pcm, char *file_name)
{
  ...
  for (n = 0; n < pcm->length; n++)
  {
    ...
    data = (unsigned char)((int)(sL + 0.5)); /* �l�̌ܓ� */
    fwrite(&data, 1, 1, fp); /* ���f�[�^�iL�`�����l���j�̏����o�� */
    ...
    data = (unsigned char)((int)(sR + 0.5)); /* �l�̌ܓ� */
    fwrite(&data, 1, 1, fp); /* ���f�[�^�iR�`�����l���j�̏����o�� */
  }
...
void wave_read_PCMU_mono(MONO_PCM *pcm, char *file_name)
{
  ...
  pcm->fs = samples_per_sec; /* �W�{�����g�� */
  pcm->bits = 16; /* �ʎq�����x */
  pcm->length = sample_length; /* ���f�[�^�̒��� */
  pcm->s = calloc(pcm->length, sizeof(double)); /* �������̊m�� */
  
  for (n = 0; n < pcm->length; n++)
  {
    fread(&c, 1, 1, fp); /* ���k�f�[�^�̓ǂݎ�� */
    ...
    pcm->s[n] = (double)s / 32768.0; /* ���f�[�^��-1�ȏ�1�����͈̔͂ɐ��K������ */
  }
...
void wave_write_PCMU_mono(MONO_PCM *pcm, char *file_name)
{
  ...
  for (n = 0; n < pcm->length; n++)
  {
    ...
    s = (short)((int)(x + 0.5) - 32768); /* �l�̌ܓ��ƃI�t�Z�b�g�̒��� */
    ...
    fwrite(&c, 1, 1, fp); /* ���k�f�[�^�̏����o�� */
  }
  
  if ((pcm->length % 2) == 1) /* ���k�f�[�^�̒�������̂Ƃ� */
  {
    c = 0;
    fwrite(&c, 1, 1, fp); /* 0�p�f�B���O */
  }
  ...
  for (block = 0; block < number_of_block; block++)
  {
    ...
    for (n = 0; n < samples_per_block; n++)
    {
      ...
      else
      {
        if ((n % 2) == 1)
        {
          fread(&data, 1, 1, fp); /* ���k�f�[�^�̓ǂݎ�� */
          
          c = (unsigned char)(data & 0x0F); /* data�̉���4bit */
        }
        else
        {
          c = (unsigned char)((data >> 4) & 0x0F); /* data�̏��4bit */
        }
      ...
      pcm->s[offset + n] = (double)s / 32768.0; /* ���f�[�^��-1�ȏ�1�����͈̔͂ɐ��K������ */
    }
  }
...
void wave_write_IMA_ADPCM_mono(MONO_PCM *pcm, char *file_name)
{
  ...
  for (block = 0; block < number_of_block; block++)
  {
    ...
    for (n = 0; n < samples_per_block; n++)
    {
      ...
      if (block == 0 && n == 0)
      {
        index = 0; /* �ŏ��̃u���b�N�ɂ�����index�̏����l��0�ɂ��� */
      }
      
      if (n == 0)
      {
        header[0] = (unsigned char)(s & 0x00FF); /* s�̉��ʃo�C�g */
        header[1] = (unsigned char)((s >> 8) & 0x00FF); /* s�̏�ʃo�C�g */
        header[2] = (unsigned char)index; /* �C���f�b�N�X */
        header[3] = 0;
        
        fwrite(header, 1, 4, fp); /* �w�b�_�̏����o�� */
      ...
      else
      {
        ...
        if ((n % 2) == 1)
        {
          data = c & 0xF; /* data�̉���4bit */
        }
        else
        {
          data |= (c & 0xF) << 4; /* data�̏��4bit */
          
          fwrite(&data, 1, 1, fp); /* ���k�f�[�^�̏����o�� */
        }
      }
    }
  }
